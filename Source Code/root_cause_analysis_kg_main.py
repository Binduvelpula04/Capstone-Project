...
!pip install torch sentence-transformers -q
!pip install plotly kaleido -q
!pip -q uninstall -y transformers
!pip -q install "transformers<5" -U # We used a Transformers version earlier than 5 to ensure compatibility with the text-to-text generation pipeline required for the FLAN-T5 model.
...
client = OpenAI()   # now it will find the env var
print("Env key loaded:", bool(os.getenv("OPENAI_API_KEY")))
